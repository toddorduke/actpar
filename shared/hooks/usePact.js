...
export function usePact(userId, activePactId = null) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (memberRows.length === 0) {
      ...
      const { data: open } = await getSupabaseClient()
        .from('pacts')
    ...
    setPosts(postsData ?? []);
    setLoading(false);
  }, [userId, activePactId, fetchMyPacts, fetchPactData]);
  ...
  const toggleOpen = useCallback(async (isOpen) => {
    if (!pact) return;
    const { error } = await getSupabaseClient().from('pacts').update({ is_open: isOpen }).eq('id', pact.id);
    if (!error) {
  ...
  const deleteRule = useCallback(async (ruleId) => {
    const { error } = await getSupabaseClient().from('pact_rules').delete().eq('id', ruleId);
  ...
  const updateMemberRole = useCallback(async (targetUserId, role) => {
    if (!pact) return { error: new Error('No pact selected') };
    const { error } = await getSupabaseClient()
      .from('pact_members')
      .update({ role })
      .eq('pact_id', pact.id)
      .eq('user_id', targetUserId);
    if (!error) setMembers((prev) => prev.map((m) => m.user_id === targetUserId ? { ...m, role } : m));
    return { error };
  }, [pact]);
  ...
  const deletePact = useCallback(async () => {
    if (!pact) return { error: new Error('No pact selected') };
    const { error } = await getSupabaseClient().from('pacts').delete().eq('id', pact.id);
    if (!error) {
      setPact(null);
      setMyPacts((prev) => prev.filter((p) => p.id !== pact.id));
      setMembers([]); setRules([]); setPosts([]);
