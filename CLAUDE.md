...
**Setup**: `npm install`, run from the repo root *or* from `client`/`mobile` directly (npm detects the workspace root automatically either way — `cd client && npm install` is exactly what the root `npm run build` script does, and what Vercel runs), resolves `@actpar/shared` and hoists it into the root `node_modules`.

**There must be exactly one `package-lock.json`, at the repo root.** `client/` and `mobile/` must NOT have their own — a nested lockfile alongside the root one (even a stale/harmless-looking one) is a known npm bug trigger: npm's optional-dependency resolution for platform-specific native binaries (Rollup's `@rollup/rollup-<platform>` packages, which Vite's build depends on) can silently fail under that setup, throwing `MODULE_NOT_FOUND` on `rollup/dist/native.js` at build time on whatever platform wasn't the one the lockfile happened to be generated on. This broke the real Vercel deploy on 2026-09-16 (Linux build, lockfile pair created on macOS) within the same session the workspace was introduced — root-caused and fixed by deleting `client/package-lock.json`/`mobile/package-lock.json` and reinstalling clean from the root. If this resurfaces, check `git status` for a stray per-package lockfile before anything else.
...
| **Ink-blue** | `--color-trust` #1E3A5F, `--color-trust-end` #16293F | Trust. Reserved for moments where the user hands something over: payment/upgrade CTAs, verified badges, saving a journal/reflection entry — and, since 2026-09-18, The Pact in full (web's nav icon + [PactPage.css](client/src/pages/Pact/PactPage.css), and mobile's [PactScreen.js](mobile/src/screens/PactScreen.js)), a deliberate extension: small invite-only accountability circles are a trust relationship, not a generic action. If it's not one of those, it isn't this color. |
